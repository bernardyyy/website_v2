...
function setupCustomCursor() {
  ...
  // Ensure the cursor is visible as soon as the script runs.
  // (Relying on `document` mouseenter/mouseleave can be unreliable across browsers.)
  cursor.style.opacity = '1';

  // Update cursor position.
  // Use pointer events when available; keep mousemove as fallback.
  ...
  links.forEach((link) => {
    link.addEventListener('mouseenter', addGrow);
    link.addEventListener('mouseleave', removeGrow);
    link.addEventListener('pointerenter', addGrow);
    link.addEventListener('pointerleave', removeGrow);
  });

  // Add event listeners for text and images
  const contentElements = document.querySelectorAll('p, h1, h2, h3, img');
  const addBlend = () => cursor.classList.add('blend');
  const removeBlend = () => cursor.classList.remove('blend');
  contentElements.forEach((element) => {
    element.addEventListener('mouseenter', addBlend);
    element.addEventListener('mouseleave', removeBlend);
    element.addEventListener('pointerenter', addBlend);
    element.addEventListener('pointerleave', removeBlend);
  });

  // Hide cursor when the tab isn't active anymore (instead of relying on `document` mouseleave).
  window.addEventListener('blur', () => {
    cursor.style.opacity = '0';
  });
  document.addEventListener('visibilitychange', () => {
    cursor.style.opacity = document.visibilityState === 'visible' ? '1' : '0';
  });
}

// Run both functions when DOM is fully loaded
const runCursorAndNavigation = () => {
  setupProjectNavigation();
  setupCustomCursor();
};
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', runCursorAndNavigation);
} else {
  runCursorAndNavigation();
}
